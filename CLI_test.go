		...
		for i, c := range cases {
			t.Run(fmt.Sprintf("%d scheduled for %v", c.expectedAmount, c.expectedScheduleTime), func(t *testing.T) {
				if len(blindAlerter.alerts) <= i {
					t.Fatalf("alert %d was NOT scheduled for %v", c.expectedAmount, c.expectedScheduleTime)
				}
				...
				if gotAmount != c.expectedAmount {
					t.Errorf("got amount %d but want %d", gotAmount, c.expectedAmount)
				}

				gotScheduleTime := alert.scheduledAt
				if gotScheduleTime != c.expectedScheduleTime {
					t.Errorf("got scheduled time of %v, but want %v", gotScheduleTime, c.expectedScheduleTime)
				}
			})
		}
	})
